...
T = 500                   # inerval of time between t = [0, T]
...
for i in range(N - 1):
    for j in range(N):
        kc_x_diag_1[i * N + j] = vx_coeff_function(x_range[i + 1],
                                                   x_range[j + 1],
                                                   k, l) / (2 * dh)
kc_x_diag_1_neg = np.zeros(N * N - N)
for i in range(N - 1):
    for j in range(N):
        kc_x_diag_1_neg[i * N + j] = -vx_coeff_function(x_range[i + 2],
                                                        x_range[j + 1],
                                                        k, l) / (2 * dh)
kc_x = sp.sparse.diags([kc_x_diag_1_neg, kc_x_diag_1],
                       [-N, N], shape=(N * N, N * N))
...
for i in range(N):
    for j in range(N - 1):
        kc_y_diag_1[i * N + j] = vy_coeff_function(x_range[i + 1],
                                                   x_range[j + 1],
                                                   k, l) / (2 * dh)
kc_y_diag_1_neg = np.zeros(N * N - 1)
for i in range(N):
    for j in range(N - 1):
        kc_y_diag_1_neg[i * N + j] = -vy_coeff_function(x_range[i + 1],
                                                        x_range[j + 2],
                                                        k,
                                                        l) / (2 * dh)
kc_y = sp.sparse.diags([kc_y_diag_1_neg, kc_y_diag_1], [-1, 1],
                       shape=(N * N, N * N))
...
for n in range(Nt):
    ...
    if n == 0:
        ...
        fig.savefig("Q_13_t=0.pdf")
    if n == 500 - 1:
        ax.set_title('u(t=50,x,y)')
        fig.savefig("Q_13_t=50.pdf")
    if n == 1000 - 1:
        ax.set_title('u(t=100,x,y)')
        fig.savefig("Q_13_t=100.pdf")
    if n == Nt - 1:
        ax.set_title('u(t=500,x,y)')
        fig.savefig("Q_13_t=500.pdf")
